...
DATA_FILE = 'consumer_data.json'
CONSUMPTION_FILE = 'consumption_samples.json'
...
async def get_stats():
    ...
    avg_risk = sum([d['risk_score'] for d in data]) / total
...
async def get_consumer_detail(consumer_id: str):
    ...
    # Return consumer details and their consumption series
    return {
        **consumer,
...
@app.get("/api/logs")
async def get_logs():
    return load_json(DATA_FILE)[:100] # Return first 100 for logs

@app.get("/api/model-performance")
async def get_performance():
    # Mocked performance metrics based on the data
    return {
        "accuracy": 0.924,
        "precision": 0.871,
        "recall": 0.846,
